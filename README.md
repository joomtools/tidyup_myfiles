# Tidyup my files
[Deutsch/German](#deutsche-anleitung)  |  [Englisch/English](#english-manual) 
...
The parameters `delete=1` and `rename=1` can be used independently or together, but one of them must be specified._
...
### Additional parameters
  ...
  If this value is not set, only the file names in the database are searched for and renamed._

  `all=1` rename all files websafe - **\[default: 0\]***<br>
  `rename=1` must be used in connection to the all parameter<br>
  Will be ignored if `delete=1` is used.

  `seo=1` Rename all files websafe **and** SEO safe - **\[default: 0\]**<br>
 `rename=1` must be used in connection to the seo parameter.<br>
  Instead of allowing underscores `_` and `CamelCase`, everything is lower case and `_` is converted to `-`.<br>
  Also converts the paths when `path=1` is used._

  `folder=images/banner` folder in the Joomla root directory to search for files - **\[default: images\]**
  ...
  Each specified extension is automatically searched in upper case letters too.
  ...
  As long as the debug=off parameter is not used, it is only a simulation, so nothing will be processed, just simulated._
